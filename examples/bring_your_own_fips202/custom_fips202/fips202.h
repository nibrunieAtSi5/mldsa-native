...
typedef struct
{
   ...
   * mldsa-native adheres to the FIPS202 state machine. You can safely
                                           ...
                                           const uint8_t *in, size_t inlen)
{
  ...
  shake_update(&state->ctx, in, inlen);
}
                                            ...
                                            mld_shake128ctx *state)
{
  ...
  shake_out(&state->ctx, out, outlen);
}
...
{
  memset(state, 0, sizeof(mld_shake128ctx));
...
{
  memset(state, 0, sizeof(mld_shake256ctx));
  state->state = FIPS202_STATE_RESET;
}
